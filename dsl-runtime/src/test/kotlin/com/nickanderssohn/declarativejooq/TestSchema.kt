...
// ---------------------------------------------------------------------------
// DocTable — composite primary key (multi-column FK parent for tests)
// ---------------------------------------------------------------------------

class DocTable private constructor() : TableImpl<DocRecord>(
    DSL.name("doc")
) {
    val ORG_ID: TableField<DocRecord, Long?> =
        createField(DSL.name("org_id"), SQLDataType.BIGINT.nullable(false), this)

    val DOC_ID: TableField<DocRecord, Long?> =
        createField(DSL.name("doc_id"), SQLDataType.BIGINT.identity(true).nullable(false), this)

    val TITLE: TableField<DocRecord, String?> =
        createField(DSL.name("title"), SQLDataType.VARCHAR(255).nullable(false), this)

    override fun getRecordType(): Class<DocRecord> = DocRecord::class.java

    override fun getPrimaryKey(): UniqueKey<DocRecord> =
        Internal.createUniqueKey(this, DSL.name("pk_doc"), arrayOf(ORG_ID, DOC_ID), true)

    override fun getIdentity(): Identity<DocRecord, *> =
        Internal.createIdentity(this, DOC_ID)

    override fun getReferences(): List<ForeignKey<DocRecord, *>> = emptyList()
...
class DocRecord() :
    UpdatableRecordImpl<DocRecord>(DocTable.DOC) {

    var orgId: Long?
        get() = get(DocTable.DOC.ORG_ID)
        set(value) = set(DocTable.DOC.ORG_ID, value)

    var docId: Long?
        get() = get(DocTable.DOC.DOC_ID)
        set(value) = set(DocTable.DOC.DOC_ID, value)

    var title: String?
        get() = get(DocTable.DOC.TITLE)
        set(value) = set(DocTable.DOC.TITLE, value)
}

// ---------------------------------------------------------------------------
// DocRevisionTable — composite FK to doc(org_id, doc_id)
// ---------------------------------------------------------------------------

class DocRevisionTable private constructor() : TableImpl<DocRevisionRecord>(
    DSL.name("doc_revision")
) {
    val ID: TableField<DocRevisionRecord, Long?> =
        createField(DSL.name("id"), SQLDataType.BIGINT.identity(true), this)

    val ORG_ID: TableField<DocRevisionRecord, Long?> =
        createField(DSL.name("org_id"), SQLDataType.BIGINT.nullable(false), this)

    val DOC_ID: TableField<DocRevisionRecord, Long?> =
        createField(DSL.name("doc_id"), SQLDataType.BIGINT.nullable(false), this)

    val SUMMARY: TableField<DocRevisionRecord, String?> =
        createField(DSL.name("summary"), SQLDataType.VARCHAR(255).nullable(false), this)

    override fun getRecordType(): Class<DocRevisionRecord> = DocRevisionRecord::class.java

    override fun getPrimaryKey(): UniqueKey<DocRevisionRecord> =
        Internal.createUniqueKey(this, DSL.name("pk_doc_revision"), arrayOf(ID), true)

    override fun getIdentity(): Identity<DocRevisionRecord, *> =
        Internal.createIdentity(this, ID)

    override fun getReferences(): List<ForeignKey<DocRevisionRecord, *>> = listOf(
        Internal.createForeignKey(
            this,
            DSL.name("fk_doc_revision_doc"),
            arrayOf(ORG_ID, DOC_ID),
            DocTable.DOC.primaryKey,
            arrayOf(DocTable.DOC.ORG_ID, DocTable.DOC.DOC_ID),
            false
        )
    )

    companion object {
        val DOC_REVISION = DocRevisionTable()
    }
}

class DocRevisionRecord() :
    UpdatableRecordImpl<DocRevisionRecord>(DocRevisionTable.DOC_REVISION) {

    var id: Long?
        get() = get(DocRevisionTable.DOC_REVISION.ID)
        set(value) = set(DocRevisionTable.DOC_REVISION.ID, value)

    var orgId: Long?
        get() = get(DocRevisionTable.DOC_REVISION.ORG_ID)
        set(value) = set(DocRevisionTable.DOC_REVISION.ORG_ID, value)

    var docId: Long?
        get() = get(DocRevisionTable.DOC_REVISION.DOC_ID)
        set(value) = set(DocRevisionTable.DOC_REVISION.DOC_ID, value)

    var summary: String?
        get() = get(DocRevisionTable.DOC_REVISION.SUMMARY)
        set(value) = set(DocRevisionTable.DOC_REVISION.SUMMARY, value)
}
    ...
    fun createDslContext(): DSLContext {
        ...
        ctx.execute(
            ...
            CREATE TABLE IF NOT EXISTS doc (
                ...
                doc_id BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
                title VARCHAR(255) NOT NULL,
        ...
        ctx.execute(
            ...
            CREATE TABLE IF NOT EXISTS doc_revision (
                ...
                FOREIGN KEY (org_id, doc_id) REFERENCES doc(org_id, doc_id)
            )
            """.trimIndent()
        )
