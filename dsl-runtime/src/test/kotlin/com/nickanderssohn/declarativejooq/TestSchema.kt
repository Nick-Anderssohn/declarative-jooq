...
// ---------------------------------------------------------------------------
// DepartmentTable (composite PK: organization_id + department_id, FK to organization)
// ---------------------------------------------------------------------------

class DepartmentTable private constructor() : TableImpl<DepartmentRecord>(DSL.name("department")) {
    val ORGANIZATION_ID: TableField<DepartmentRecord, Long?> =
        createField(DSL.name("organization_id"), SQLDataType.BIGINT.nullable(false), this)
    val DEPARTMENT_ID: TableField<DepartmentRecord, Long?> =
        createField(DSL.name("department_id"), SQLDataType.BIGINT.nullable(false), this)
    val NAME: TableField<DepartmentRecord, String?> =
        createField(DSL.name("name"), SQLDataType.VARCHAR(255).nullable(false), this)

    override fun getRecordType(): Class<DepartmentRecord> = DepartmentRecord::class.java

    override fun getPrimaryKey(): UniqueKey<DepartmentRecord> =
        Internal.createUniqueKey(this, DSL.name("pk_department"), arrayOf(ORGANIZATION_ID, DEPARTMENT_ID), true)

    override fun getReferences(): List<ForeignKey<DepartmentRecord, *>> = listOf(
        Internal.createForeignKey(
            this, DSL.name("fk_department_organization"), arrayOf(ORGANIZATION_ID),
            OrganizationTable.ORGANIZATION.primaryKey, arrayOf(OrganizationTable.ORGANIZATION.ID), false
        )
    )
...
class DepartmentRecord() : UpdatableRecordImpl<DepartmentRecord>(DepartmentTable.DEPARTMENT) {
    var organizationId: Long?
        get() = get(DepartmentTable.DEPARTMENT.ORGANIZATION_ID)
        set(value) = set(DepartmentTable.DEPARTMENT.ORGANIZATION_ID, value)
    var departmentId: Long?
        get() = get(DepartmentTable.DEPARTMENT.DEPARTMENT_ID)
        set(value) = set(DepartmentTable.DEPARTMENT.DEPARTMENT_ID, value)
    var name: String?
        get() = get(DepartmentTable.DEPARTMENT.NAME)
        set(value) = set(DepartmentTable.DEPARTMENT.NAME, value)
}

// ---------------------------------------------------------------------------
// EmployeeTable (composite FK: organization_id + department_id -> department)
// ---------------------------------------------------------------------------

class EmployeeTable private constructor() : TableImpl<EmployeeRecord>(DSL.name("employee")) {
    val ID: TableField<EmployeeRecord, Long?> =
        createField(DSL.name("id"), SQLDataType.BIGINT.identity(true), this)
    val NAME: TableField<EmployeeRecord, String?> =
        createField(DSL.name("name"), SQLDataType.VARCHAR(255).nullable(false), this)
    val ORGANIZATION_ID: TableField<EmployeeRecord, Long?> =
        createField(DSL.name("organization_id"), SQLDataType.BIGINT.nullable(false), this)
    val DEPARTMENT_ID: TableField<EmployeeRecord, Long?> =
        createField(DSL.name("department_id"), SQLDataType.BIGINT.nullable(false), this)

    override fun getRecordType(): Class<EmployeeRecord> = EmployeeRecord::class.java

    override fun getPrimaryKey(): UniqueKey<EmployeeRecord> =
        Internal.createUniqueKey(this, DSL.name("pk_employee"), arrayOf(ID), true)

    override fun getIdentity(): Identity<EmployeeRecord, *> =
        Internal.createIdentity(this, ID)

    override fun getReferences(): List<ForeignKey<EmployeeRecord, *>> = listOf(
        Internal.createForeignKey(
            this, DSL.name("fk_employee_department"),
            arrayOf(ORGANIZATION_ID, DEPARTMENT_ID),
            DepartmentTable.DEPARTMENT.primaryKey,
            arrayOf(DepartmentTable.DEPARTMENT.ORGANIZATION_ID, DepartmentTable.DEPARTMENT.DEPARTMENT_ID),
            false
        )
    )
...
class EmployeeRecord() : UpdatableRecordImpl<EmployeeRecord>(EmployeeTable.EMPLOYEE) {
    var id: Long?
        get() = get(EmployeeTable.EMPLOYEE.ID)
        set(value) = set(EmployeeTable.EMPLOYEE.ID, value)
    var name: String?
        get() = get(EmployeeTable.EMPLOYEE.NAME)
        set(value) = set(EmployeeTable.EMPLOYEE.NAME, value)
    var organizationId: Long?
        get() = get(EmployeeTable.EMPLOYEE.ORGANIZATION_ID)
        set(value) = set(EmployeeTable.EMPLOYEE.ORGANIZATION_ID, value)
    var departmentId: Long?
        get() = get(EmployeeTable.EMPLOYEE.DEPARTMENT_ID)
        set(value) = set(EmployeeTable.EMPLOYEE.DEPARTMENT_ID, value)
}
    ...
    fun createDslContext(): DSLContext {
        ...
        ctx.execute(
            ...
            CREATE TABLE IF NOT EXISTS employee (
                ...
                department_id   BIGINT NOT NULL,
                FOREIGN KEY (organization_id, department_id) REFERENCES department(organization_id, department_id)
            )
            """.trimIndent()
        )
